...
stage1_qc = function(main_path, flair_name, mimosa_name, subject, cores = 1){
  flair_files = list.files(main_path, pattern = flair_name, recursive = TRUE, full.names = TRUE)
  flair_files = flair_files[which(grepl(subject, flair_files))]
  ...
  mimosa_files = mimosa_files[which(grepl(subject, mimosa_files))]
