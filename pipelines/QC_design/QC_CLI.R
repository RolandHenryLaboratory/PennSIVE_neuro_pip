...
p <- add_argument(p, "--range", short = '-r', help = "specify the index range for all the files to be included.", default = "1-50")
...
source(paste0(argv$app, "/image_tool.R"))
source(paste0(argv$app, "/qc_prep.R"))
...
if(argv$stage == "prep"){
  ...
  qc_list = stage1_qc(main_path = main_path, flair_name = flair_name, mimosa_name = mimosa_name, index_range = index_range, cores = n)
  message('Saving outputs...')
  saveRDS(qc_list, out.dir)
}else{
  message('Checking inputs...')
